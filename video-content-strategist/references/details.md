...
1. **How-to tutorials** -- step-by-step implementation (highest search volume) → verify: step output matches expected outcome
2. **Tool reviews and comparisons** -- evaluation content (high commercial intent) → verify: step output matches expected outcome
3. **Case studies and teardowns** -- authority building (highest trust) → verify: step output matches expected outcome
4. **Opinion and hot takes** -- algorithm-friendly, shareable → verify: step output matches expected outcome
...
1. **Identify source** -- which blog/podcast/webinar has the highest traffic or engagement? → verify: step output matches expected outcome
2. **Extract the hook** -- what is the single most compelling insight or result? → verify: step output matches expected outcome
3. **Write the short script** -- 60 seconds max, hook, value, CTA → verify: output exists + parses without error
4. **Adapt for each platform** -- same core, different framing and caption style → verify: step output matches expected outcome
5. **Schedule for staggered release** -- do not publish same content on all platforms same day → verify: step output matches expected outcome
