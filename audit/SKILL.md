---
name: audit
description: "Run technical quality checks across accessibility, performance, theming, responsive design, and anti-patterns. Triggers: 'use audit', 'audit', 'audit task'."
...
1. **[P?] `/command-name`** — Brief description (specific context from audit findings) → verify: findings count > 0 OR clean signal returned
2. **[P?] `/command-name`** — Brief description (specific context) → verify: step output matches expected outcome
...
- Task is unrelated to audit — pick a domain-specific skill instead
...
| "Generic template is good enough" | Audit needs domain-specific judgment, not boilerplate |
...
Done when:
- Primary deliverable produced matches user's stated goal for audit
...
- Input: standard user request involving audit
