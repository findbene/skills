...
### Scaling Methods
1. **Vertical**: increase budget on winning campaigns (20% rule) → verify: step output matches expected outcome
2. **Horizontal**: duplicate winning campaigns to new audiences → verify: step output matches expected outcome
3. **Platform expansion**: add budget on new platforms → verify: dependency resolves + import works
4. **Geographic expansion**: test new markets/regions → verify: all checks pass
5. **Format expansion**: test new ad formats on same platform → verify: all checks pass
