...
1. **Consistency** — Post on schedule. Algorithms reward reliability. → verify: step output matches expected outcome
2. **Engagement bait done right** — Genuine questions, not "like if you agree." Polls work. Hot takes work. Asking for opinions works. → verify: user confirms
3. **Collaboration** — Co-create content with complementary accounts. → verify: output exists + parses without error
4. **Repurposing** — One blog post → 5-10 social posts across platforms.
5. **Trend riding** — Jump on relevant trends fast, but only if authentic to your brand. → verify: step output matches expected outcome
6. **Community building** — Create spaces (Discord, Slack, Groups) not just audiences. → verify: output exists + parses without error
