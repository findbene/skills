...
description: "Data engineering skill for building scalable data pipelines, ETL/ELT systems, and data infrastructure. Triggers: 'use senior-data-engineer', 'senior data engineer', 'senior-data-engineer task'."
...
1. [Trigger Phrases](#trigger-phrases) → verify: step output matches expected outcome
2. [Quick Start](#quick-start) → verify: step output matches expected outcome
...
4. [Architecture Decision Framework](#architecture-decision-framework) → verify: step output matches expected outcome
5. [Tech Stack](#tech-stack) → verify: step output matches expected outcome
6. [Reference Documentation](#reference-documentation) → verify: step output matches expected outcome
...
- Task is unrelated to senior data engineer — pick a domain-specific skill instead
...
| "Generic template is good enough" | Senior Data Engineer needs domain-specific judgment, not boilerplate |
...
Done when:
- Primary deliverable produced matches user's stated goal for senior data engineer
...
- Input: standard user request involving senior data engineer
