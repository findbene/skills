...
1. **Write at 5th-grade reading level** — short sentences, simple words → verify: file content matches expected shape
2. **One action per step** — no compound instructions → verify: step output matches expected outcome
3. **Start each step with action verbs** — "Click", "Open", "Verify", "Enter"
4. **Include expected results** — tell users what they should see → verify: step output matches expected outcome
5. **Add warnings for critical steps** — prevent common mistakes → verify: dependency resolves + import works
6. **Create quality checks** — define "done" explicitly → verify: output exists + parses without error
...
### Step 1: [Action Title]
1. [Do this specific action] → verify: step output matches expected outcome
2. [Do this specific action] → verify: step output matches expected outcome
