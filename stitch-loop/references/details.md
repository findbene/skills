...
2. If `stitch.json` exists, use stored `projectId` — do not create a new project → verify: output exists + parses without error
3. If no `stitch.json`, call `create_project` → save numeric ID to `stitch.json`
4. Call `generate_screen_from_text`: → verify: output exists + parses without error
...
5. Call `get_screen` with numeric projectId + screenId → verify: step output matches expected outcome
6. Download HTML: `bash scripts/fetch-stitch.sh "[htmlCode.downloadUrl]" "queue/[page].html"` → verify: file content matches expected shape
7. Download screenshot: save to `queue/[page].png` → verify: file content matches expected shape

### Step 4 — Integrate into site

1. Move `queue/[page].html` → `site/public/[page].html`
2. Fix asset paths (make relative to `site/public/`) → verify: diff matches intended change
3. Wire navigation: replace `href="#"` placeholders with real paths to existing pages → verify: step output matches expected outcome
4. Ensure the header/footer matches other pages in the site → verify: step output matches expected outcome

### Step 4.5 — Visual verification (if Chrome DevTools MCP available)

If `chrome*` tools are in `list_tools`:
1. Start local server: `npx serve site/public -p 3000` → verify: step output matches expected outcome
2. Navigate to `http://localhost:3000/[page].html` → verify: step output matches expected outcome
3. Take screenshot, compare against `queue/[page].png` → verify: step output matches expected outcome
4. Stop server → verify: step output matches expected outcome
...
2. Write `next-prompt.md` with: → verify: output exists + parses without error
...
1. index.html — main landing page → verify: step output matches expected outcome
2. about.html — company/team info → verify: step output matches expected outcome
3. contact.html — contact form → verify: step output matches expected outcome
