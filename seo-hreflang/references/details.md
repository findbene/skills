...
### Process
1. **Detect languages**: Scan site for language indicators (URL path, subdomain, TLD, HTML lang attribute) → verify: findings count > 0 OR clean signal returned
2. **Map page equivalents**: Match corresponding pages across languages/regions → verify: step output matches expected outcome
3. **Validate language codes**: Verify all codes against ISO 639-1 and ISO 3166-1
4. **Generate tags**: Create hreflang tags for each page including self-referencing → verify: output exists + parses without error
5. **Verify return tags**: Confirm all relationships are bidirectional
6. **Add x-default**: Set fallback for each page set → verify: dependency resolves + import works
7. **Output**: Generate implementation code (HTML, HTTP headers, or sitemap XML) → verify: output exists + parses without error
