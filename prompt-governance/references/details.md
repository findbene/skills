...
1. **BRANCH** -- Create feature branch for prompt change → verify: output exists + parses without error
2. **DEVELOP** -- Edit prompt in dev environment, manual testing → verify: all checks pass
3. **EVAL** -- Run eval pipeline vs. golden dataset (automated in CI) → verify: command exit code 0
4. **COMPARE** -- Compare new prompt eval score vs. current production score → verify: step output matches expected outcome
5. **REVIEW** -- PR review: eval results plus diff of prompt changes → verify: step output matches expected outcome
6. **PROMOTE** -- Staging to Production with approval gate → verify: step output matches expected outcome
7. **MONITOR** -- Watch production metrics for 24-48h post-deploy → verify: step output matches expected outcome
8. **ROLLBACK** -- One-command rollback to previous version if needed → verify: step output matches expected outcome
