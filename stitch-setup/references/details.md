...
1. Go to [stitch.withgoogle.com/settings](https://stitch.withgoogle.com/settings) → verify: step output matches expected outcome
2. Scroll to the **API Keys** section → verify: step output matches expected outcome
3. Click **"Create API Key"**
4. Copy the key — you'll need it in the next step → verify: step output matches expected outcome
...
Expected behavior:
1. `stitch-orchestrator` activates → verify: step output matches expected outcome
2. It runs `list_tools` and finds Stitch MCP tools → verify: command exit code 0
3. It generates a Design Spec → verify: output exists + parses without error
4. It creates a project via `create_project` → verify: output exists + parses without error
5. It generates a screen via `generate_screen_from_text` → verify: output exists + parses without error
6. It retrieves the screen via `get_screen` → verify: step output matches expected outcome
7. It asks you which framework to convert to → verify: user confirms
