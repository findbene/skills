...
1. Rank all options by IRR (highest first) → verify: step output matches expected outcome
2. Fund in order until budget is exhausted → verify: step output matches expected outcome
3. Exception: fund anything with payback <6 months first (quick wins) → verify: step output matches expected outcome
4. Never fund negative NPV unless strategic reason — name it explicitly → verify: step output matches expected outcome
...
**RISKS TO WATCH:**
1. [Risk + mitigation] → verify: step output matches expected outcome
2. [Risk + mitigation] → verify: step output matches expected outcome
