...
### Recommendations
1. Convert X images to WebP format (est. XX KB savings) → verify: step output matches expected outcome
2. Add alt text to X images → verify: dependency resolves + import works
3. Add dimensions to X images → verify: dependency resolves + import works
4. Enable lazy loading on X below-fold images → verify: file content matches expected shape
5. Compress X oversized images → verify: step output matches expected outcome
