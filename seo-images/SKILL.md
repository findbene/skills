...
name: seo-images
description: >
  Image SEO and performance optimization analysis. Audits alt text quality, file
  sizes by category (thumbnails <50KB, content images <100KB, hero images <200KB),
  format recommendations (WebP/AVIF over JPEG/PNG), responsive images with srcset
  and picture element, lazy loading implementation, CLS prevention (width/height
  attributes), and Google Image Search eligibility. Flags missing alt text, oversized
  images, outdated formats, and layout shift risks. Make sure to use this skill
  whenever the user mentions: "image optimization", "alt text", "image SEO", "image
  file size", "image audit", "images not loading fast", "WebP", "lazy loading",
  "image alt tags", "accessibility images", "CLS from images", "image search traffic",
  "responsive images", "picture element", "image compression", or any concern about
  images on a website — whether it's speed, accessibility, SEO, or format.
user-invokable: true
argument-hint: "[url]"
allowed-tools:
  - Read
...
Flag images exceeding thresholds and recommend compression to target sizes without quality loss.
...
Recommend WebP/AVIF over JPEG/PNG. Check for `<picture>` elements with format fallbacks and flag sites that lack them.
