...
**主要功能：**
- 生成针对 6-60 秒格式优化的完整视频脚本
- 理解趋势音频、趋势标签和算法时间
- 在 2 秒、5 秒、10 秒、15 秒及以上时间构建保留机制
- 创建具有精确时间的文本覆盖、切割和过渡的脚本
- 针对特定平台的宽高比进行优化（9:16 垂直以适应移动优先）
- 从规划阶段整合声音设计策略
- 建议缩略图帧和关键时刻以获得最大影响力
- 输出为已准备好的编辑软件格式（DaVinci Resolve、CapCut、Adobe Premiere）
...
**1. 模式中断**
大脑是一台预测机器。 When something violates the pattern, the brain says "WAIT. That's anomalous. Pay attention."
- A glitch in reality
...
**2. 好奇心间隙**
当大脑检测到缺失信息时，它想填补空隙。 钩子在观众的脑海中创建了一个问题，直到他们继续观看才能回答。
...
**3. 情感触发**
情感胜过逻辑。 The amygdala (emotion center) processes stimuli faster than the prefrontal cortex (logic center). When you trigger emotion in the first 2 seconds, you bypass the conscious "should I keep scrolling?" decision.
...
**4. 对比原则**
人类通过比较来感知。 A quiet sound followed by loud sound registers as LOUDER than if the loud sound played alone. A slow motion cut followed by normal speed looks FASTER. 这是感觉对比，它劫持了注意力。
...
**5. 运动检测**
进化使我们有能力注意运动。 In a field of static images, movement grabs attention in 200 milliseconds (faster than conscious thought). 第一帧或第一个过渡必须包括运动—not stationary talking heads.
- Fast cuts
...
These are production-ready Seedance 2.0（Higgsfield） prompts. Each 15-25 lines, each obsessed with the first 2 seconds.
...
When using Seedance 2.0（Higgsfield） for hook generation, expect the following output format:
...
### When Prompting Seedance 2.0（Higgsfield）:
...
Use Seedance 2.0（Higgsfield）. Follow the framework. Trust the data. Ship the video. Measure the results. Iterate. Repeat.
