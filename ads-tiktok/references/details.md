...
1. Test 3-5 hooks per winning concept → verify: all checks pass
2. Rotate creatives every 5-7 days (fatigue sets in fast) → verify: step output matches expected outcome
3. Kill underperformers after 3 days if CTR <0.5% → verify: step output matches expected outcome
4. Scale winners by duplicating (not increasing budget on same ad) → verify: step output matches expected outcome
5. Repurpose winning concepts, not assets (fresh footage, same angle) → verify: step output matches expected outcome
