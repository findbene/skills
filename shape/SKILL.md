---
name: shape
description: "Plan the UX and UI for a feature before writing code. Runs a structured discovery interview, then produces a design brief that guides implementation. Triggers: 'use shape', 'shape', 'shape task'."
...
- Task is unrelated to shape — pick a domain-specific skill instead
...
| "Generic template is good enough" | Shape needs domain-specific judgment, not boilerplate |
...
Done when:
- Primary deliverable produced matches user's stated goal for shape
...
- Input: standard user request involving shape
