...
Built on [Claude Banana](https://github.com/AgriciDaniel/banana-claude),
the standalone AI image generation skill for Claude Code.

Two components with distinct roles:
...
Served via **mcp-proxy** (`nanobanana-mcp`) with `GOOGLE_AI_API_KEY` already configured.
No installation needed — fully active now.

**Check availability:** Verify the MCP server is connected by checking if
`gemini_generate_image` or `set_aspect_ratio` tools are available in the current
tool list. If not available, check that `nanobanana` is listed in
`~/.claude/mcp-proxy/config.json` and that `GOOGLE_AI_API_KEY` is set.
...
1. **Identify use case** from command or context (og, hero, product, etc.) → verify: step output matches expected outcome
2. **Apply SEO defaults** from the use cases table above → verify: diff matches intended change
3. **Set aspect ratio** via `set_aspect_ratio` MCP tool → verify: step output matches expected outcome
4. **Construct Reasoning Brief** using the banana Creative Director pipeline: → verify: dependency resolves + import works
...
5. **Generate** via `gemini_generate_image` MCP tool → verify: output exists + parses without error
6. **Post-generation SEO checklist** (see below) → verify: step output matches expected outcome
...
1. **Alt text**:Write descriptive, keyword-rich alt text for the generated image → verify: output exists + parses without error
2. **File naming**:Rename to SEO-friendly format: `keyword-description-widthxheight.webp` → verify: step output matches expected outcome
3. **WebP conversion**:Convert to WebP for optimal page speed: → verify: step output matches expected outcome
   ```bash
   magick output.png -quality 85 output.webp
   ```
4. **File size**:Target under 200KB for hero images, under 100KB for thumbnails → verify: step output matches expected outcome
5. **Schema markup**:Suggest `ImageObject` schema for the generated image: → verify: output exists + parses without error
   ```json
   {
     "@type": "ImageObject",
   ...
   ```
6. **OG meta tags**:For social preview images, remind about: → verify: step output matches expected outcome
   ```html
...
Image generation costs money — be transparent about it:
...
- Run `cost_tracker.py summary` when the user asks about usage
...
| Error | Resolution |
|-------|-----------|
| MCP not configured | Verify `nanobanana` entry exists in `~/.claude/mcp-proxy/config.json` with `GOOGLE_AI_API_KEY` set |
...
1. **Image path**:where it was saved → verify: step output matches expected outcome
2. **Crafted prompt**:show what was sent to the API (educational) → verify: step output matches expected outcome
3. **Settings**:model, aspect ratio, resolution → verify: step output matches expected outcome
4. **SEO checklist**:alt text suggestion, file naming, WebP conversion → verify: step output matches expected outcome
5. **Schema snippet**:ImageObject or og:image markup if applicable → verify: step output matches expected outcome
