...
1. **Creative concept** (different messaging angles, not just color changes) → verify: step output matches expected outcome
2. **Hook/first 3 seconds** (video opening on Meta, TikTok, YouTube) → verify: file content matches expected shape
3. **Offer structure** (pricing, discount type, free trial length) → verify: step output matches expected outcome
4. **Landing page** (headline, CTA, form length) → verify: step output matches expected outcome
5. **Bidding strategy** (tCPA vs tROAS vs Maximize Conversions) → verify: step output matches expected outcome

### Medium Impact
6. **Audience targeting** (interest vs lookalike vs broad) → verify: step output matches expected outcome
7. **Ad format** (static vs video vs carousel) → verify: step output matches expected outcome
8. **CTA button** (Learn More vs Sign Up vs Shop Now) → verify: step output matches expected outcome
9. **Campaign structure** (CBO vs ABO, consolidated vs segmented) → verify: step output matches expected outcome

### Low Impact (test last)
10. **Ad scheduling** (time of day, day of week) → verify: step output matches expected outcome
11. **Device targeting** (mobile vs desktop) → verify: step output matches expected outcome
12. **Minor copy variations** (word substitutions without concept change) → verify: step output matches expected outcome
