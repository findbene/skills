...
description: "Monitors customer health, predicts churn risk, and identifies expansion opportunities using weigh. Triggers: 'use customer-success-manager', 'customer success manager', 'customer-success-manager task."
...
1. **Combine signals**: Use all three scripts together for a complete customer picture → verify: step output matches expected outcome
2. **Act on trends, not snapshots**: A declining Green is more urgent than a stable Yellow → verify: step output matches expected outcome
3. **Calibrate thresholds**: Adjust segment benchmarks based on your product and industry per `references/health-scoring-framework.md` → verify: step output matches expected outcome
4. **Prepare with data**: Run scripts before every QBR and executive meeting; reference `references/cs-playbooks.md` for intervention guidance → verify: command exit code 0
...
churn, customer health scores, upsell opportunities, expansion revenue, retention analysis, or customer analytics. Runs three Python CLI tools to produce deterministic health scores, churn risk t...

## When NOT to use

- Task is unrelated to customer success manager — pick a domain-specific skill instead
...
| "Generic template is good enough" | Customer Success Manager needs domain-specific judgment, not boilerplate |
...
Done when:
- Primary deliverable produced matches user's stated goal for customer success manager
...
- Input: standard user request involving customer success manager
