...
description: 'Scalable frontend design system creation for consistent UI components, typography, spacing scales, and token-based th. Triggers: "use frontend-design-system", "frontend design system", "frontend task.'
...
1. **Intentional Design** - Every choice serves a purpose → verify: step output matches expected outcome
2. **Systematic Thinking** - Components fit together logically → verify: step output matches expected outcome
3. **Progressive Complexity** - Simple base, powerful composition → verify: step output matches expected outcome
4. **Accessibility First** - Inclusive by default → verify: step output matches expected outcome
5. **Performance Conscious** - Fast and efficient → verify: step output matches expected outcome
...
1. Document everything — component API, usage examples → verify: step output matches expected outcome
2. Version tokens — track breaking changes → verify: step output matches expected outcome
3. Test accessibility — automated and manual → verify: all checks pass
4. Performance audit — bundle size, render time → verify: findings count > 0 OR clean signal returned
5. Consistent naming — clear, predictable conventions → verify: step output matches expected outcome
6. Composable patterns — flexible building blocks → verify: step output matches expected outcome
7. Theme support — light/dark modes at minimum → verify: step output matches expected outcome

## When NOT to use

- Task is unrelated to frontend design system — pick a domain-specific skill instead
...
| "Generic template is good enough" | Frontend Design System needs domain-specific judgment, not boilerplate |
...
Done when:
- Primary deliverable produced matches user's stated goal for frontend design system
...
- Input: standard user request involving frontend design system
