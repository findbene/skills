...
description: 'This skill should be used when the user asks to ''optimize prompts'', ''design prompt templates'', ''evaluate LLM outputs'', ''build agentic systems'', ''implement RAG'', ''create few-shot examples''.'
...
- Task is unrelated to senior prompt engineer — pick a domain-specific skill instead
...
| "Generic template is good enough" | Senior Prompt Engineer needs domain-specific judgment, not boilerplate |
...
Done when:
- Primary deliverable produced matches user's stated goal for senior prompt engineer
...
- Input: standard user request involving senior prompt engineer
