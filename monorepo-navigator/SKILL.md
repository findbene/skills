...
description: "Navigate, analyze, and optimize monorepos using Turborepo, Nx, pnpm workspaces, or Lerna - cross-package impact analysis, selective builds/tests on affected packages only, remote caching, dependency graph visualization, and multi-repo migration. Use any time you are working in a monorepo, builds are slow, or packages share code. Trigger on: 'monorepo', 'turborepo', 'nx workspace', 'pnpm workspace', 'affected packages', 'which packages changed', 'monorepo build', 'multi-repo migration'."
