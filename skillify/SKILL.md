---
name: skillify
description: Convert a video transcript summary or any content into a reusable, well-structured Claude skill (SKILL.md). Use this skill whenever the user wants to turn knowledge from a transcript, summary, article, or conversation into a lasting Claude skill file. Apply whenever the user says "turn this into a skill", "skillify this", "make a skill from this", "create a Claude skill from this transcript", "convert this summary to a skill", "capture this as a skill", "make this knowledge reusable", "build a skill out of this", "save this as a skill", or "I want to be able to reuse this knowledge". Also trigger when the user has just finished a bulk or transcribe session and wants to extract skills from the summaries. Do not just explain what a skill is — immediately start the skillify workflow to create the SKILL.md file.
user_invocable: true
---
...
Create the skill directory and SKILL.md file following Anthropic's skill-creator v2.0 format:
...
---
name: {skill-name}
description: {One-line description}. Use when {trigger conditions}. Apply whenever {specific phrases or contexts}. Make the description pushy enough that Claude will trigger on it without being explicitly asked.
