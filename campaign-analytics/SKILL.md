...
description: "Analyzes campaign performance with multi-touch attribution, funnel conversion analysis, and ROI calculation for mar. Triggers: 'use campaign-analytics', 'campaign analytics', 'campaign-analytics task."
...
1. **Use multiple attribution models** -- Compare at least 3 models to triangulate channel value; no single model tells the full story. → verify: step output matches expected outcome
2. **Set appropriate lookback windows** -- Match your time-decay half-life to your average sales cycle length. → verify: step output matches expected outcome
3. **Segment your funnels** -- Compare segments (channel, cohort, geography) to identify performance drivers. → verify: step output matches expected outcome
4. **Benchmark against your own history first** -- Industry benchmarks provide context, but historical data is the most relevant comparison. → verify: step output matches expected outcome
5. **Run ROI analysis at regular intervals** -- Weekly for active campaigns, monthly for strategic review. → verify: command exit code 0
6. **Include all costs** -- Factor in creative, tooling, and labor costs alongside media spend for accurate ROI. → verify: step output matches expected outcome
7. **Document A/B tests rigorously** -- Use the provided template to ensure statistical validity and clear decision criteria. → verify: all checks pass
...
- Task is unrelated to campaign analytics — pick a domain-specific skill instead
...
| "Generic template is good enough" | Campaign Analytics needs domain-specific judgment, not boilerplate |
...
Done when:
- Primary deliverable produced matches user's stated goal for campaign analytics
...
- Input: standard user request involving campaign analytics
