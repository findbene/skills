...
description: "Analyzes competitor products and companies by synthesizing data from pricing pages, app store reviews, job po. Triggers: 'use competitive-teardown', 'competitive teardown', 'competitive-teardown task."
...
1. **Define competitors** — List 2–4 competitors to analyze. Confirm which is the primary focus. → verify: step output matches expected outcome
2. **Collect data** — Use `references/data-collection-guide.md` to gather raw signals from at least 3 sources per competitor (website, reviews, job postings, SEO, social).   → verify: step output matches expected outcome
...
3. **Score using rubric** — Apply the 12-dimension rubric below to produce a numeric scorecard for each competitor and your own product.   → verify: diff matches intended change
   _Validation checkpoint: Every dimension should have a score and at least one supporting evidence note._
4. **Generate outputs** — Populate the templates in `references/analysis-templates.md` (Feature Matrix, Pricing Analysis, SWOT, Positioning Map, UX Audit). → verify: output file exists + no syntax error
5. **Build action plan** — Translate findings into the Action Items template (quick wins / medium-term / strategic). → verify: step output matches expected outcome
6. **Package for stakeholders** — Assemble the Stakeholder Presentation using outputs from steps 3–5. → verify: step output matches expected outcome
...
1. **Executive Summary** — Threat level (LOW/MEDIUM/HIGH/CRITICAL), top strength, top opportunity, recommended action → verify: step output matches expected outcome
2. **Market Position** — 2x2 positioning map → verify: step output matches expected outcome
3. **Feature Scorecard** — 12-dimension radar or table, total scores → verify: step output matches expected outcome
4. **Pricing Analysis** — Comparison table + key insight → verify: step output matches expected outcome
5. **UX Highlights** — What they do better (3 bullets) vs. where we win (3 bullets) → verify: step output matches expected outcome
6. **Voice of Customer** — Top 3 review complaints (quoted or paraphrased) → verify: step output matches expected outcome
7. **Our Action Plan** — Quick wins, medium-term, strategic priorities; Appendix with raw data → verify: step output matches expected outcome
...
- Task is unrelated to competitive teardown — pick a domain-specific skill instead
...
| "Generic template is good enough" | Competitive Teardown needs domain-specific judgment, not boilerplate |
...
Done when:
- Primary deliverable produced matches user's stated goal for competitive teardown
...
- Input: standard user request involving competitive teardown
