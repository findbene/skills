...
1. Parse the URL: → verify: step output matches expected outcome
...
2. Call `[prefix]:get_screen` with those IDs → verify: diff matches intended change
3. Skip ahead to Step 2 → verify: step output matches expected outcome
...
4. Call `[prefix]:get_screen` with numeric `projectId` and `screenId` → verify: diff matches intended change
...
1. **More Stitch screens:** Copy Section 6 into the DESIGN SYSTEM block of your next Stitch prompt. → verify: step output matches expected outcome
   This keeps every new screen visually consistent with your existing design.

2. **Multi-page build with stitch-loop:** Include Section 6 in every `next-prompt.md` baton file. → verify: step output matches expected outcome

3. **Code conversion:** Hand off to `stitch-design-system` when you're ready to generate → verify: file content matches expected shape
