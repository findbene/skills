---
name: "demo-video"
description: 'Use when the user asks to create a demo video, product walkthrough, feature showcase, animated presentation, marketing video, or GIF from screens. Triggers: "use demo-video", "demo video", "demo task.'
...
1. `scenes/` — one HTML file per scene (1920x1080 viewport) → verify: step output matches expected outcome
2. `narration/` — one `.txt` file per scene (for edge-tts input) → verify: step output matches expected outcome
3. `scenes.json` — manifest listing scenes in order with durations and narration text → verify: step output matches expected outcome
4. `build.sh` — shell script that runs the full pipeline: → verify: command exit code 0
...
- Task is unrelated to demo video — pick a domain-specific skill instead
...
| "Generic template is good enough" | Demo Video needs domain-specific judgment, not boilerplate |
...
Done when:
- Primary deliverable produced matches user's stated goal for demo video
...
- Input: standard user request involving demo video
