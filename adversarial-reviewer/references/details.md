...
1. [Quick Start](#quick-start) → verify: step output matches expected outcome
2. [Review Workflow](#review-workflow) → verify: step output matches expected outcome
3. [The Three Personas](#the-three-personas) → verify: step output matches expected outcome
4. [Severity Classification](#severity-classification) → verify: step output matches expected outcome
5. [Output Format](#output-format) → verify: step output matches expected outcome
6. [Anti-Patterns](#anti-patterns)
7. [When to Use This](#when-to-use-this) → verify: step output matches expected outcome
...
**Review Process:**
1. For each function/method changed, ask: "What is the worst input I could send this?" → verify: user confirms
2. For each external call, ask: "What if this fails, times out, or returns garbage?" → verify: user confirms
3. For each state mutation, ask: "What if this runs twice? Concurrently? Never?" → verify: command exit code 0
4. For each conditional, ask: "What if neither branch is correct?" → verify: user confirms
...
**Review Process:**
1. Read each changed function as if you've never seen the codebase. Can you understand what it does from the name, parameters, and body alone? → verify: file content matches expected shape
2. Trace one code path end-to-end. How many files do you need to open? → verify: file content matches expected shape
3. Check: would a new contributor know where to add a similar feature? → verify: dependency resolves + import works
4. Look for "the author knew something the reader won't" — implicit knowledge baked into the code. → verify: file content matches expected shape
...
**Review Process:**
1. Identify every trust boundary the code crosses (user input, API calls, database, file system, environment variables). → verify: step output matches expected outcome
2. For each boundary: is input validated? Is output sanitized? Is the principle of least privilege followed? → verify: all checks pass
3. Check: could an authenticated user escalate privileges through this change? → verify: step output matches expected outcome
4. Check: does this change expose any new attack surface? → verify: step output matches expected outcome
...
1. Read the code **bottom-up** (start from the last function, work backward). → verify: file content matches expected shape
2. For each function, state its contract **before** reading the body. Does the body match? → verify: file content matches expected shape
3. Assume every variable could be null/undefined until proven otherwise. → verify: step output matches expected outcome
4. Assume every external call will fail. → verify: step output matches expected outcome
5. Ask: "If I deleted this change entirely, what would break?" — if the answer is "nothing," the change might be unnecessary. → verify: user confirms
